...
int Daq_servernumber = 0;
string MonitorZmqTCPHost = "localhost";
int MonitorZmqTCPPort = 5555;
...
void *sendMonitorDataZmq( void *arg)
{
    char *host =(char*)MonitorZmqTCPHost.data();
    if ( getenv("MONITORZMQTCPHOST")  )
      {
        host = getenv("MONITORZMQTCPHOST");
      }

    if ( getenv("MONITORZMQTCPPORT")  )
      {
        MonitorZmqTCPPort = atoi(getenv("MONITORZMQTCPPORT"));
      }

    zmq::socket_t* server;
    if ( getenv("PUSHPULL")  )
      {
        std::cout<<"sendMonitorDataZmq: PUSHPULL scheme set"<<std::endl;
        server=new zmq::socket_t(*zmqctx, zmq::socket_type::push);
      }else{
        server=new zmq::socket_t(*zmqctx, zmq::socket_type::pub);
    }

    char tmpcmd[500];
    sprintf(tmpcmd,"tcp://%s:%d",host, MonitorZmqTCPPort);
    server->connect(tmpcmd);
    //server->connect("ipc:///tmp/0");
    std::cout<<"connected to ZMQ tcp socket for monitoring: "<<tmpcmd<<std::endl;

    char sendid[200];
    sprintf(sendid,"M%d",Daq_servernumber);
    int sendidlen=strlen(sendid);
    while (1) {
        pthread_mutex_lock( &SendSem);

        //! sender
        transportBuffer->sendDataZmq(server,Buffer_number, sendid,sendidlen);
        //---end sender
...
int rcdaq_init(zmq::context_t* ctx, pthread_mutex_t &M, int servernumber)
{

    if ( getenv("ENABLE_EMPTY_EVENT")  )
      {
        enable_empty_event_flag = 1;
      }
...
//    }
  ...
  //  daq_webcontrol ( ThePort+servernumber, outputstream);
  daq_webcontrol ( ThePort+servernumber);
...
int daq_webcontrol(const int port, std::ostream& os)
{
  ...
  if (status ) 
    ...
    {
      os << "web service created on port " << ThePort << endl;
      return 0;
    }
  return 0;
