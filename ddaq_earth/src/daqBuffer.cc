...
unsigned int daqBuffer::sendDataZmq(zmq::socket_t *zmqsock,int buffer_number, char* sendid,int sendidlen)
{
  ...
  //! send envelope
  zmqsock->send(zmq::buffer(sendid,sendidlen), zmq::send_flags::sndmore);
  //! send buffer number
  zmq::send_result_t status = zmqsock->send(zmq::buffer((char *) &buffer_number, 4),zmq::send_flags::sndmore);
  //! send total size
  status = zmqsock->send(zmq::buffer((char *) &total, 4),zmq::send_flags::sndmore);
  char *p = (char *) bptr;
  zmq::send_result_t sent = zmqsock->send(zmq::buffer(p, total));
  //cout<<"VAL = "<<sent.value()<<"\t"<<total<<endl;
  return (unsigned int)sent.value();
}
