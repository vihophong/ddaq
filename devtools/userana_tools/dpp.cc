...
void dpp::fast_maxADC_thr(UShort_t&ledtime,UShort_t&adcpos,UShort_t&adc,UShort_t* Waveline,Int_t nsamples,Int_t start,UShort_t absolute_thr) //work only for positive pulse
{
    ledtime = 0;
    adc = 0;
    adcpos = 0;
    for (int i=start;i<nsamples;i++){
        if (Waveline[i]>absolute_thr&&ledtime==0) ledtime = i;
        if (Waveline[i]>adc) {
            adcpos = i;
            adc = Waveline[i];
        }
