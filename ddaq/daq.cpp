                           ...
                           for(unsigned int ev=0; ev<NumEvents_t[ch]; ev++){
                                  ...
                                  if (shmp->block_no>(uint32_t)MAX_BLOCK-1) {
                                      ...
                                      */
                                      ...
                                      //clear is fail!!!
                                      //clear buffer here it can not be clear that easy
                               ...
                               for (size_t i=0;i<save_Length;i++){
                                   shmp->buffer[shmp->block_no].wfdata[i]=WaveLine[i];
                                   //Int_t ypoint=shmp->buffer[shmp->block_no].wfdata[i];
                                   //if (ypoint<=1000&&shmp->buffer[shmp->block_no].wfdata[i+1]==0&&i<save_Length-1){
                                   //    strcpy(shmp->com_message,"stop");
                                   //    goto end_run;
                                   //}
                               }
                               */
                               ...
                               /*
                               //no data processing (it not affect the performance and without it code can not run?)
                               dpp *oj=new dpp((unsigned int)Waveform_t->Ns,WaveLine);
                               delete oj;
                               */
